...
DOCKER_IMAGE := ksauraj/k8s-telegram-bot
DOCKER_TAG := $(VERSION)
...
build: ## Build the binary
	...
	@CGO_ENABLED=0 go build $(LDFLAGS) -o $(BINARY_NAME) ./cmd/k8sbot
	@echo "Build complete: $(BINARY_NAME)"

build-all: ## Build for all platforms
	@echo "Building for all platforms..."
	@GOOS=linux GOARCH=amd64 CGO_ENABLED=0 go build $(LDFLAGS) -o dist/$(BINARY_NAME)-linux-amd64 ./cmd/k8sbot
	@GOOS=linux GOARCH=arm64 CGO_ENABLED=0 go build $(LDFLAGS) -o dist/$(BINARY_NAME)-linux-arm64 ./cmd/k8sbot
	@GOOS=darwin GOARCH=amd64 CGO_ENABLED=0 go build $(LDFLAGS) -o dist/$(BINARY_NAME)-darwin-amd64 ./cmd/k8sbot
	@GOOS=darwin GOARCH=arm64 CGO_ENABLED=0 go build $(LDFLAGS) -o dist/$(BINARY_NAME)-darwin-arm64 ./cmd/k8sbot
	@GOOS=windows GOARCH=amd64 CGO_ENABLED=0 go build $(LDFLAGS) -o dist/$(BINARY_NAME)-windows-amd64.exe ./cmd/k8sbot
...
install: build ## Install binary to GOPATH/bin
	@go install $(LDFLAGS) ./cmd/k8sbot

run: build ## Build and run the bot (requires config)
	@./$(BINARY_NAME) --config config.yaml

run-dev: ## Run with development settings
	@TELEGRAM_BOT_TOKEN=$(TELEGRAM_BOT_TOKEN) KUBECONFIG=$(KUBECONFIG) go run ./cmd/k8sbot --log-level debug
